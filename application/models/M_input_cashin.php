...
class M_input_cashin extends CI_Model {
	...
	public function view_inputan($tgl){
		$result = $this->db->query("SELECT * FROM tbl_cashinproj WHERE tanggal='$tgl' ORDER BY kode_status");
		return $result->result_array();
	}

	public function view_inputan_collection($tgl){
		$result = $this->db->query("SELECT * FROM tbl_cashinproj WHERE tanggal='$tgl' AND (kode_status='C' OR kode_status='D') ORDER BY kode_status");
		return $result->result_array();
	}

	public function view_inputan_collection_default(){
		$tanggal_sekarang = date('d-m-Y');
		$result = $this->db->query("SELECT * FROM tbl_cashinproj WHERE tanggal = '$tanggal_sekarang' AND (kode_status='C' OR kode_status='D')");
		return $result->result_array();
	}
