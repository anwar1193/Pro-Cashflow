...
if(isset($_FILES['berkas_excel']['name']) && in_array($_FILES['berkas_excel']['type'], $file_mimes)) {
	...
	for($i = 1;$i < count($sheetData);$i++)
	{
        ...
        $q_cek_proj = "SELECT * FROM tbl_cashinproj WHERE kode_status='$kode_status' AND tanggal='$tanggal'";
        ...
        if($cek_proj>0){ //Jika Sudah Ada Proyeksinya
            ...
            $q_cek = "SELECT * FROM tbl_cashinreal WHERE kode_status='$kode_status' AND tanggal='$tanggal'";
            ...
            if($cek>0){ //kalo data sudah ada sebelumnya
                ...
                mysqli_query($koneksi, "UPDATE tbl_cashinreal SET realisasi=$real_baru WHERE kode_status='$kode_status' AND tanggal='$tanggal'");

                // Masukan Detail Data
                mysqli_query($koneksi, "insert into tbl_cashinreal_d(kode_status, status, tanggal, realisasi) 
                values ('$kode_status','$status','$tanggal', $realisasi)");

            }else{
                // Masukan Data
                mysqli_query($koneksi, "insert into tbl_cashinreal(kode_status, status, tanggal, realisasi) 
                values ('$kode_status','$status','$tanggal', $realisasi)");
    ...
    // echo '<script>
    //     alert("Data Berhasil Diupload");window.location="http://10.20.0.30/Pro-Cashflow/input_cashin_real";
    // </script>';

    // Redirect Di Laptop Local
    //  echo '<script>
    //     alert("Data Berhasil Diupload");window.location="http://localhost:8056/Pro-Cashflow/input_cashin_real";
    // </script>';

    // Redirect Di Server 10.20.1.201 (Yang Ada Domain nya)
    echo '<script>
        alert("Data Berhasil Diupload");window.location="http://10.20.0.30/Pro-Cashflow/collection_realisasi";
    </script>';
