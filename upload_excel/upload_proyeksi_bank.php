...
if(isset($_FILES['berkas_excel']['name']) && in_array($_FILES['berkas_excel']['type'], $file_mimes)) {
	...
	for($i = 1;$i < count($sheetData);$i++)
	{
        ...
        $q_cek = "SELECT * FROM tbl_cashoutproj WHERE kode_status='$kode_status' AND tanggal='$tanggal'";
        ...
        if($cek>0){ //kalo data sudah ada sebelumnya
            ...
            mysqli_query($koneksi, "UPDATE tbl_cashoutproj SET projection=$proj_baru WHERE kode_status='$kode_status' AND tanggal='$tanggal'");

            // Masukan Detail Data
            mysqli_query($koneksi, "insert into tbl_cashoutproj_d(kode_status, status, tanggal, projection) 
    ...
    // echo '<script>
    //     alert("Data Berhasil Diupload");window.location="http://10.20.0.30/Pro-Cashflow/input_cashout";
    // </script>';

    // Redirect Di Laptop Local
    //  echo '<script>
    //     alert("Data Berhasil Diupload");window.location="http://localhost:8056/Pro-Cashflow/input_cashout";
    // </script>';

    // Redirect Di Server 10.20.1.201 (Yang Ada Domain nya)
    echo '<script>
        alert("Data Berhasil Diupload");window.location="http://localhost/Pro-Cashflow/bank_proyeksi";
    </script>';
